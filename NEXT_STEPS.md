...
- A targeted CLIP diagnostic resolved the prospective CLIP gap without changing
  that boundary. Re-embedding the same 90 fresh replication clips with 8 sampled
  frames and prompt-text CLIP similarities still did not accept generated-video
...
| P1 | Recognition stimulus production manifest | Turns the recognition design into concrete seed-image requests and SVD generation jobs without accepting near-duplicate lures | Seed subgate passed: 60/60 seed PNGs present, 5 screening sheets produced, first-pass image distinctiveness accepted for SVD generation; 60 output MP4s still missing | `content_pocket_recognition_stimulus_production_manifest_20260608.json` and `.md` | Done; generate SVD MP4s next |
...
- `descriptor_conditioned_replication_clip_diagnostic_result_20260608.md`
  reran the embedding readout with 8 sampled CLIP frames and prompt-text
  similarities. Generated-video CLIP still failed (`clip_video` AUC 0.6667,
...
- `content_pocket_recognition_stimulus_production_manifest_20260608.json` and
  ...
  Current status is `seed_images_screened_ready_for_svd_generation`.
...
1. Generate the 60 SVD MP4s listed in the production manifest from the screened
   seed images under `data/recognition_memory_seed_images_20260608/`, then
   build video contact sheets and screen every MP4 before freezing the launch
   set.
