"""Train TRIBE → CLIP-image adapter and check memorability-direction alignment.

Hypothesis: T5 text embeddings of BMD captions don't carry memorability signal
(cos(high-mean, low-mean) = 0.9999 in T5 space). CLIP-image embeddings — which
see actual frames — should. If so, this is the right substrate for
brain-direction-conditioned video generation via image-conditioned T2V models.
"""
...
def main() -> None:
    print("[adapter-clip] loading CLIP image embeddings")
    clip_data = np.load("data/features/clip_image_embeddings.npz", allow_pickle=False)
    clip_embeds = np.asarray(clip_data["embeddings"], dtype=np.float32)
    clip_sids = list(np.asarray(clip_data["sample_ids"]).tolist())
    print(f"  CLIP: {clip_embeds.shape}")
    bmd = _bmd()
    ...
    for i, sid in enumerate(clip_sids):
        ...
        if not f.exists():
            continue
        rows.append({"sid": sid, "i": i, "tribe": _load_tribe(f),
                     "clip": clip_embeds[i], "mem": bmd[vid]})
    if not rows:
        raise SystemExit("No (tribe, clip-image, mem) triples found — check CLIP sample_ids encoding")
    ...
    print(f"  X (TRIBE): {X.shape}, Y (CLIP): {Y.shape}, mems: {mems.shape}")

    # === First, the key diagnostic: is the memorability direction in CLIP space non-zero? ===
    order = np.argsort(mems)
    ne = int(len(mems) * 0.30)
    ...
    high_norm = np.linalg.norm(y_high_mean)
    low_norm = np.linalg.norm(y_low_mean)
    cos_means = float(np.dot(y_high_mean, y_low_mean) / (high_norm * low_norm + 1e-12))
    pct = 100 * diff_norm / high_norm

    print()
    print(f"[clip-space] cos(high-mean, low-mean) = {cos_means:.4f}")
    print(f"[clip-space] ||high-low|| / ||high|| = {pct:.2f}% (T5 was 1.2%)")
    print()

    device = torch.device("mps" if torch.backends.mps.is_available() else
                          "cuda" if torch.cuda.is_available() else "cpu")
    print(f"[adapter-clip] device={device}")
    ...
    v_mem_clip_native = diff / (diff_norm + 1e-12)
    ...
    v_mem_clip_via_adapter = hi - lo
    v_mem_clip_via_adapter /= (np.linalg.norm(v_mem_clip_via_adapter) + 1e-12)
    cos_alignment = float(np.dot(v_mem_clip_native, v_mem_clip_via_adapter))

    print()
    print(f"[result] cos(v_mem_clip_native, v_mem_clip_via_adapter) = {cos_alignment:+.4f}")
    print(f"         (compare: T5 = -0.187; MiniLM (§6.2) = +0.936)")

    out = Path("data/reports/adapter_tribe_to_clip.pt")
    torch.save({
        "state_dict": adapter.state_dict(),
        "config": {"in_dim": int(X.shape[1]), "hidden": 1024, "out_dim": int(Y.shape[1])},
        "v_mem_tribe": v_mem_tribe,
        "v_mem_clip_native": v_mem_clip_native,
        "v_mem_clip_via_adapter": v_mem_clip_via_adapter,
        "cos_alignment": cos_alignment,
        "cos_means_high_low": cos_means,
        "diff_pct_of_norm": pct,
        "encoder": "CLIP-ViT-L-14 image (768-dim mean over 4 frames)",
    }, out)

    sidecar = Path("data/reports/adapter_tribe_to_clip.json")
    sidecar.write_text(json.dumps({
        ...
        "diff_pct": float(pct),
        "cos_alignment_v_native_v_adapter": float(cos_alignment),
    }, indent=2))
