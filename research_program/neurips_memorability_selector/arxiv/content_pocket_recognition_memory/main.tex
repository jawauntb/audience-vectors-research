...
Code, paper sources, participant-safe aggregate outputs, analysis scripts, and
study/stimulus manifests are maintained in the
\href{https://github.com/jawauntb/audience-vectors-research}{project repository}.
The key committed
reproducibility artifacts are the frozen recognition-memory design, stimulus
production and screening manifests, hosted-video URL map, response-analysis
plan, aggregate Wave 2 result JSON/Markdown, figure exports, and the script that
recomputes the reported statistics from private raw exports.

Raw Prolific exports and webhook payloads are not committed because they include
participant metadata. The generated video stimuli are not embedded as a PDF
appendix; for external review or archival release they should be distributed as
a separate stimulus artifact bundle, ideally with stable URLs or a DOI, together
with the existing manifests and file hashes. This keeps the paper readable while
still allowing independent scrutiny of the exact old and lure clips.
...
result and design artifacts include:
\begin{itemize}
  \item \path{content_pocket_recognition_memory_design_20260608.json}
  \item \path{content_pocket_validation_stimuli_manifest_20260608.json}
  \item \path{content_pocket_recognition_video_screening_20260608.json}
  \item \path{content_pocket_recognition_hosted_video_url_map_20260608.json}
  \item \path{content_pocket_recognition_response_analysis_plan_20260608.json}
